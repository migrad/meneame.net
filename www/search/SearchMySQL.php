...
class SearchMySQL implements iSearch
{
    ...
    public function search($by_date = false, $start = 0, $count = 50)
    {
        ...
        else if ($_REQUEST['w'] === 'posts') {
            return $this->db_get_search_posts($by_date, $start, $count);
        }
        else {
    ...
    private function db_get_search_links($by_date = false, $start = 0, $count = 50)
    {
        ...
        switch ($_REQUEST['p']) {
            ...
            default:
                ...
                if ($_REQUEST['w'] === 'articles') $this->where = "link_content_type = 'article' and ";
    ...
    private function db_get_search_comments($by_date = false, $start = 0, $count = 50)
    {
        ...
        if ($_REQUEST['h']) {
            $hours = (int)$_REQUEST['h'];
            $this->where .= " and comment_date > date_sub(now(), interval $hours hour)";
        }
    ...
    private function db_get_search_posts($by_date = false, $start = 0, $count = 50)
    {
        global $db;
        // For now it serves for search specific blogs (used from most voted sites)
        ...
        $this->from = 'posts';
        $this->where = "match(post_content) against ('$url' IN BOOLEAN MODE)";

        if ($_REQUEST['h']) {
            $hours = (int)$_REQUEST['h'];
            $this->where .= " and post_date > date_sub(now(), interval $hours hour)";
        }
        ...
        if ($this->where && $this->from) {
            $sql = "select post_id $this->fields from $this->from where $this->where order by $this->order limit $start,$count";
            $response['rows'] = $db->get_var("select count(*) from $this->from where $this->where");
            if ($response['rows'] > 0) {
                $response['ids'] = array();
